...
function syncDotMarkers(){
  ...
  Object.values(dotsCache).forEach(dot=>{ if(!hasValidLatLng(dot)) return; if(markerById.has(dot.id)){const m=markerById.get(dot.id);m.setLatLng([Number(dot.lat),Number(dot.lng)]);m.setStyle(dotStyle(dot.status));m.unbindTooltip();if(dot.label)m.bindTooltip(dot.label,{direction:"top",offset:[0,-6]});} else addDotMarker(dot); });
}

function hasValidLatLng(dot){ return Number.isFinite(Number(dot?.lat)) && Number.isFinite(Number(dot?.lng)); }
function addDotMarker(dot){ if(!hasValidLatLng(dot)) return null; const marker=L.circleMarker([Number(dot.lat),Number(dot.lng)],dotStyle(dot.status)).addTo(dotLayer); markerById.set(dot.id,marker); if(dot.label) marker.bindTooltip(dot.label,{direction:"top",offset:[0,-6]}); marker.on("click",()=>openDotPopup(dot.id,marker)); return marker; }
...
function doSearch(){ const q=$("searchInput").value.trim().toLowerCase(); if(!q) return; const match=Object.values(dotsCache).find(d=>hasValidLatLng(d)&&(d.label||"").toLowerCase().includes(q)); if(!match) return toast("No map match"); const marker=markerById.get(match.id); if(map) map.setView([Number(match.lat),Number(match.lng)],Math.max(map.getZoom(),17)); if(marker) marker.setStyle({...dotStyle(match.status),radius:10,weight:3}); lastFoundMarker=marker; toast(`Found: ${match.label||match.id}`); }
