...
function initMap(){
  ...
  map.on(L.Draw.Event.CREATED, async event => { const layer=event.layer; const name=prompt("Territory name?","Neighborhood")?.trim()||"Neighborhood"; const color=prompt("Color hex?","#38bdf8")?.trim()||"#38bdf8"; const id=`nb-${Date.now()}`; layer._nbId=id; layer.setStyle?.({color,weight:3,fillColor:color,fillOpacity:.12,interactive:false}); neighborhoodLayer.addLayer(layer); await setDoc(doc(db,"neighborhoods",id),{id,name,color,geojson:layer.toGeoJSON(),createdBy:currentName,createdAt:Date.now()}); await addRemoteLog(`🗺️ ${currentName} created territory: ${name}`); toast("Territory saved"); });
...
function syncNeighborhoodLayers(){
  ...
  Object.values(neighborhoodsCache).forEach(nb=>{ if(nbLayerById.has(nb.id)) return; const group=L.geoJSON(nb.geojson,{interactive:false,style:{color:nb.color,weight:3,fillColor:nb.color,fillOpacity:.12}}); const layers=[]; group.eachLayer(layer=>{neighborhoodLayer.addLayer(layer); bindNeighborhoodInteractions(nb.id,layer); layers.push(layer);}); nbLayerById.set(nb.id,layers[0]||group); });
}
...
function openEntityModal(type, seed={}){
  const configs = {
    ...
    jobModal:{title:"Job",coll:"jobs",fields:["title","customer","address","scheduledAt","cleaner","price","status","notes"],defaults:{status:"scheduled",repName:currentName,repId:currentUid},selects:{status:["scheduled","in progress","completed","cancelled"]}},
...
window.crmEdit = (kind,id) => { 
  const configs={
    ...
    job:{title:"Job",coll:"jobs",fields:["title","customer","address","scheduledAt","cleaner","price","status","notes"],selects:{status:["scheduled","in progress","completed","cancelled"]}},
